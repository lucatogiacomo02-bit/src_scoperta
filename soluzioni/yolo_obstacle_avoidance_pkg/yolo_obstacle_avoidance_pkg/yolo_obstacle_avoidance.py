...
class YoloObstacleAvoidanceNode(Node):
    ...
    def detect_obstacle(self):
        ...
        # Se il target è stato identificato, non è segnalato come ostacolo.
        if self.target_detected:
    ...
    def detect_target(self, xyxy: np.ndarray, conf: np.ndarray, cls_ids: np.ndarray, image_h: int, image_w: int,
        ...
        for i in range(len(xyxy)):
            ...
            # Prosegue solo se la classe corrente è la classe target
    ...
    def control_loop(self):
        ...
        if self.target_detected:
            ...
            if self.current_distance < self.safe_distance:
                ...
                self.get_logger().info(f"Target raggiunto ({self.current_distance:.2f}m). Task completata. Stop...")
                return                
        ...
        if self.obstacle_detected:
            ...
            # Analizza metà sinistra (escluso il settore frontale che è occupato)
            left_min = np.nanmin(self.ranges[:center - 30])
            # Analizza metà destra (escluso il settore frontale che è occupato)
            right_min = np.nanmin(self.ranges[center + 30:])
        ...
        if self.avoiding:
            dist = self.distance_from_point(self.start_avoiding_x, self.start_avoiding_y)
